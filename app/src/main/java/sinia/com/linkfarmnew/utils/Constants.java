package sinia.com.linkfarmnew.utils;

import com.a.a.method;

/**
 * Created by 忧郁的眼神 on 2016/7/26.
 */
public class Constants {

    public static String BASE_URL = "http://192.168.0.108:8080/farmer/xiNaiInterfaceController" +
            ".do?interface&sign=123&messageFormat=json/xml&locale=zh_CN&appKey=001&method=";
//    public static String BASE_URL = "http://114.55.91.34:8080/farmer/xiNaiInterfaceController" +
//            ".do?interface&sign=123&messageFormat=json/xml&locale=zh_CN&appKey=001&method=";

    public static final String BMOB_KEY = "d4ecefd1ecdd89d41b182409cd68f2d4";

    /**
     * 支付宝支付
     */
    public final static String ALIPAY_PARTNER = "2088421591030980";
    public final static String ALIPAY_SELLER = "2088421591030980";
    public final static String RSA_PRIVATE =
            "MIICdQIBADANBgkqhkiG9w0BAQEFAASCAl8wggJbAgEAAoGBAJ7geqvusA3m7dRiSedEyqXpzwlWibpmqh6ZR" +
                    "/LSWDLMgfNSa3TyZBGzs0P20mZs16AE1Iy6ts4K/hLMduVoD+x8LnBDvGm7wamSeH1GmigZHnSOLLNrXcu/5gwF" +
                    "+gQNKtrnwCTVTZLoh6VCs5VSqakE5" +
                    "/RPiyQ3Nh5E0FTIaScNAgMBAAECgYA763O4lnvlsluuRyeSRpqbOnQwFGg7We79PVgqV3SWLBUyvLn0UFq3ZmvMzTELGi" +
                    "+0lT3Imb7KNNEgK+cHnFisgwNrvaezqPchW4R+UKLZbsHf6zRgCyVms77wh" +
                    "";
}
